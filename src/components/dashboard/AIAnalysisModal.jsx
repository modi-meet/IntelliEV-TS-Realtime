...
const AIAnalysisModal = ({ isOpen, onClose, onReport }) => {
  ...
  const calculateSeverity = (predictions) => {
    let score = 0;
    let vehicleCount = 0;
        ...
        <div className="space-y-4">
          ...
          {previewUrl && (
            <div className="relative border rounded-lg overflow-hidden bg-gray-100 min-h-[200px] flex items-center justify-center">
               <img 
                 ...
                 src={previewUrl} 
                 alt="Preview" 
                 className={`max-w-full max-h-[400px] ${results ? 'hidden' : 'block'}`}
               />
               <canvas 
                 ref={canvasRef}
                 className={`max-w-full max-h-[400px] ${results ? 'block' : 'hidden'}`}
               />
            </div>
          )}
          ...
          {results && (
            <div className={`p-4 rounded-lg ${
              severity <= 3 ? 'bg-green-100 text-green-800' : 
              severity <= 6 ? 'bg-yellow-100 text-yellow-800' : 
          ...
          <div className="flex gap-3 mt-4">
            ...
            {results && (
              <button 
                ...
                className={`flex-1 py-2.5 rounded-lg text-white font-semibold transition-colors ${severity > 6 ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}
              >
